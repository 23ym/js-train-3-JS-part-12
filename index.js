...
function stringToNumber(str) {
  const number = Number(str);// Використовуємо Number() для перетворення рядка в число
  return number; // Повертаємо отримане число
}
...
function isAlmostSame(num1, num2) {
  const difference = Math.abs(num1 - num2); // Визначаємо, чи є різниця між двома числами меншою або рівною EPSILON
...
function isSafeInteger(num) {
  return num >=Number.MIN_SAFE_INTEGER && num <= Number.MAX_SAFE_INTEGER;  // Перевіряємо, чи є число меншим або рівним MAX_SAFE_INTEGER і більшим або рівним MIN_SAFE_INTEGER
}
